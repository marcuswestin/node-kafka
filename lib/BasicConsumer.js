...
module.exports = std.Class(Client, function(supr) {
	...
	this.fetchTopic = function(args) {
		...
		this._writeFetchRequest(request)
		return this
	}
	...
	this.fetchOffsets = function(args) {
		...
		this._requests.push(request)
		this._writeOffsetsRequest(request)
		return this
	}
		...
		for (i in this._requests) {
			switch (this._requests[i].type) {
				case requestTypes.FETCH:
					this._writeFetchRequest(this._requests[i])
					break;
				case requestTypes.OFFSETS:
					this._writeOffsetsRequest(this._requests[i])
					break;
			}
		}
	}
	
	this._writeFetchRequest = function(request) {
		if (this._connected) this._connection.write(this._encodeFetchRequest(request))
	}
	...
	this._writeOffsetsRequest = function(request) {
		if (this._connected) this._connection.write(this._encodeOffsetsRequest(request))
	}
		...
		while (index != buf.length) {
			...
			switch (this._state) {					
				...
				case states.RESPONSE_MSG:
					...
					if (payload != null) {
						this._requests[0].offset += payload.length + 9
						console.log(this._requests[0].offset)
						next = states.RESPONSE_MSG_0
